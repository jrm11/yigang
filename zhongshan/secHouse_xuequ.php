	...
	<body>
		...
		<div class="ny_banner">
			...
			<div class="ny_banner_d">
				<a href="index.php">首页</a> / <a href="" title="资讯">二手房</a> / <a href="secHouse_xuequ.php" title="资讯">学区房</a>
			</div>
		</div>
		<!--banner结束-->
		<!--开始找房-->
		...
		<!--精选资讯推荐-->
		<div class="secHouse_xuequ w1170">
			<div class="ind_box2">
				<!--<div class="ind_t">
					<div class="t">深圳二手学区房</div>
					<div>行业引领者和颠覆者，承诺真实房源，提供安心服务，保障消费者权益</div>
				</div>-->
				<ul>
					<?php
						$sqlcount='select count(*) from pro_co_zs where '.$sq.' ';
						$sql='select * from `'.$tablepre.'pro_co_zs` where '.$sq.' order by px desc,id desc';
					...
					?>
						<li>
							<a class="ind_box2_img" href="secHouse_show.php?id=<?php echo $row['id']; ?>" title=""><img src="../<?php echo $row['img_sl']; ?>"/></a>
							<a class="ind_box2_t" href="secHouse_show.php"><?php echo $row['title']; ?></a>
							<div>附近学校：<?php echo $row['pro_cs22']; ?></div>
							<div>价格：<?php echo $row['pro_cs43']; ?>万</div>
						</li>
					<?php
							}
						}
