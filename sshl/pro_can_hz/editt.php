...
function xialm($lm){
	...
	$sql='select `id_lm` from `'.$tablepre.'pro_can_hz` where `fid`='.$lm.'';
	$rek=$db->query($sql);
...
$sql='update `'.$tablepre.'pro_can_hz` set `fid`='.$fid.',`title_lm`="'.$title_lm.'",`add_xx`="'.$add_xx.'",`xia`="'.$xia.'",`px`='.$px.' where `id_lm`='.$id.'';
$db->execute($sql);
