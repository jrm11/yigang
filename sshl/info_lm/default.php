...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
  ...
  function deflm($fid,$i){
	...
	while($rsk=$db->getrow($rek)){
		...
		echo'<td align="center"><A href="edit.php?id='.$rsk["id_lm"].'">修改</A> | <A href="make.php?id='.$rsk["id_lm"].'&ac=del"  onClick="return confirm(\'真的要删除该栏目吗?\r\n\r\n所属该栏目的所有信息将被删除！\')" >删除</A></td>'."\n";
		//echo'<td align="center"><A href="edit.php?id='.$rsk["id_lm"].'">修改</A></td>'."\n";
		echo'</tr>'."\n";
		if ($rsk['xia']=='yes'){
			deflm($rsk['id_lm'],$i);
		}
