...
if($ac=='px'){
	...
	foreach($px as $k=>$v){
		$sql='update `'.$tablepre.'pro_can_dg` set `px`='.$v.' where `id_lm`='.$k.'';
		$db->execute($sql);
	}
}
else{	
	...
	//删除
	if ($ac=='del'){
		$sql='select `id_lm`,`xia` from `'.$tablepre.'pro_can_dg` where `id_lm` in ('.$id.')';
		$result=$db->query($sql);
		while ($row=$db->getrow($result)){
			$db->execute('delete from `'.$tablepre.'pro_can_dg` where `id_lm`='.$row['id_lm'].'');
...
function dellm($id_lm){
	global $db,$tablepre;
	$sql='select `id_lm`,`xia` from `'.$tablepre.'pro_can_dg` where `fid`='.$id_lm.'';
	$rek=$db->query($sql);
	while($rsk=$db->getrow($rek)){
		$db->execute('delete from `'.$tablepre.'pro_can_dg` where `id_lm`='.$rsk["id_lm"].'');
