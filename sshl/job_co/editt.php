...
$sql='update `'.$tablepre.'job_co` set `lm`='.$lm.',`title`="'.$title.'",`color_font`="'.$color_font.'",`uselink`="'.$uselink.'",`link_url`="'.$link_url.'",`info_from`="'.$info_from.'",`info_author`="'.$info_author.'",`f_body`="'.$f_body.'",`z_body`="'.$z_body.'",`img_sl`="'.$img_sl.'",`dow_sl`="'.$dow_sl.'",`vid_sl`="'.$vid_sl.'",`wtime`="'.$wtime.'",`px`='.$px.',`read_num`='.$read_num.' where `id`='.$id.'';
$db->execute($sql);
