...
//开始读信息
$sql='select COUNT(*) from `'.$tablepre.'job_co` a, `'.$tablepre.'job_lm` b where a.`lm`=b.`id_lm` '.$sq.'';
$counter=$db->getqueryallrow($sql);
$p=new page();
$p->setpage($counter,25);
$sql=$p->getlimit();
$sql='select a.`id`,a.`lm`,a.`title`,a.`wtime`,a.`color_font`,a.`img_sl`,a.`px`,a.`read_num`,a.`pass`,a.`tuijian`,a.`hot`,b.`title_lm` from `'.$tablepre.'job_co` a left join `'.$tablepre.'job_lm` b on a.`lm`=b.`id_lm` where `pass`<>"" '.$sq.' order by a.`px` desc,a.`id` desc'.$sql;
$result=$db->query($sql);
