...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    ...
    <select name="fid" id="fid">
      <option value="0" selected="selected">无{作为一级分类}</option>
		...
		function addlm($fid,$i){
			...
			$sql='select * from `'.$tablepre.'pro_can_zs` where `fid`='.$fid.' and `xia`="yes" order by `px` desc,`id_lm` asc';
			$rek=$db->query($sql);
  ...
  <tr class="tdbg" style="display:none;">
    ...
    <td>	<input name="add_xx" id="add_xx" type="radio" value="yes" >是
			<input name="add_xx" id="add_xx" type="radio"  value="no" checked>
		  否 可以添加产品</td>
  </tr>
    <tr class="tdbg">
    <td width="120" align="right">下级分类：</td>
    <td><input name="xia" id="xia" type="radio" value="yes" >是
		<input name="xia" id="xia" type="radio"  value="no" checked>
		  否 有下级分类</td>
  </tr>
    <tr class="tdbg">
