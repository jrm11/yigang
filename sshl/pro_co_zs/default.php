            ...
            <td>
                <select name="lm" id="lm">
                    ...
                    function addlm($fid,$i){
                        ...
                        $sql='select * from `'.$tablepre.'pro_lm_zs` where `fid`='.$fid.'  order by `px` desc,`id_lm` desc';
                        $rek=$db->query($sql);
...
function xialm($lm){
	...
	$sql='select `id_lm` from `'.$tablepre.'pro_lm_zs` where `fid`='.$lm.'';
	$rek=$db->query($sql);
	while($rsk=$db->getrow($rek)){
		$st.=','.$rsk['id_lm'];
		xialm($rsk['id_lm']);
...
//开始读产品
$sqlcount='select COUNT(*) from `'.$tablepre.'pro_co_zs` a, `'.$tablepre.'pro_lm_zs` b where a.`lm`=b.`id_lm` '.$sq.'';
$counter=$db->getqueryallrow($sqlcount);
$sql='select a.`id`,a.`lm`,a.`title`,a.`img_sl`,a.`px`,a.`read_num`,a.`pass`,a.`tuijian`,a.`hot`,a.`jing`,b.`title_lm`,b.`id_lm`,a.`huxing_lm` from `'.$tablepre.'pro_co_zs` a left join `'.$tablepre.'pro_lm_zs` b on a.`lm`=b.`id_lm` where `pass`<>"" '.$sq.' order by a.`px` desc,a.`id` desc';
$p=new page();
...
if($row['huxing_lm']){
	$sqlq='select * from pro_co_zs where id='.$row['huxing_lm'].' ';
