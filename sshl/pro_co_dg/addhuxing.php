...
<FORM name="form1" id="form1" method="post" action="addd.php" onSubmit="return check()">
	<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
		...
		function addlm($fid,$i){
			...
			$sql='select * from `'.$tablepre.'pro_lm_dg` where `fid`='.$fid.' order by `px` desc,`id_lm` desc';
			$rek=$db->query($sql);
		...
		<tr class="tdbg">
			...
			<td><select name="huxing" id="huxing">
					...
					<?php
						$sql_ser='select * from `'.$tablepre.'pro_co_dg` where `lm`=47 order by px desc,id desc  ';
